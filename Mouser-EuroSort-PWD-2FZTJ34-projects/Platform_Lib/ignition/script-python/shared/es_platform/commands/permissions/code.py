...
class CommandAuthorizer(object):
	...
	def require(self, eventType, userId=None, context=None):
		...
		# 2) Else fall back to the operator’s roles (Joe stays logged in)
...
def _roles_match_any(roles, required_any):
	# If rule requires roles but list is empty, treat as deny.
	if not required_any:
		return False

	role_set = set([_norm_role(r) for r in (roles or []) if r is not None])
	for req in (required_any or []):
		if _norm_role(req) in role_set:
			return True
	return False
...
def _authenticate(username, password, user_source, logger=None):
	...
	# 1) Ignition 8+: system.security.authenticate(...)
	# Signature varies; try common forms.
	try:
		if hasattr(system, "security") and hasattr(system.security, "authenticate"):
			try:
				# common: (username, password, userSource)
				return bool(system.security.authenticate(str(username), str(password), us))
			except:
				# alternate: (userSource, username, password)
				return bool(system.security.authenticate(us, str(username), str(password)))
	except Exception as e:
